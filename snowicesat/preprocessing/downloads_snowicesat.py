...
def crop_metadata_to_glacier(gdir):
    """
    A function that reads Sentinel metadata (solar
     zenith angle and solar azimuth angle) in .tif format from
    cache folder for different dates
    and processes them to the size of the each glacier and
    reprojects to local grid.
    The output is then stored in the solar_angles.nc file with the
    dimensions "angles"  with the current time stamp

    Structure of the function:
    - Reading Metadata Data from cache folder in Working Directory (bandwise)
    - Cropping  MetaData to glacier outline
    - Reprojecting Raster of Glacier to local grid
    - Resampling all bands to 10 Meter resolution
    - Writing local raster of all bands to multi-temporal
       netCDF file solar_angles.nc
    ...
    ----------
        None
    """
    print("In crop_metadata_to_glacier")
    glacier = gpd.read_file(gdir.get_filepath('outlines'))
    img_path = os.path.join(os.path.join(
                        cfg.PATHS['working_dir'],
                        'cache',str(cfg.PARAMS['date'][0]),
    ...
    for band in os.listdir(img_path):
        with rasterio.open(os.path.join(img_path, band)) as src:
            # Read crs from Metadata Tile:
            if band == "solar_azimuth_angles.tif":
                local_crs = glacier.crs
                glacier = glacier.to_crs(src.crs)
                glacier.to_file(driver='ESRI Shapefile',
                                filename=os.path.join(cfg.PATHS['working_dir'],
            ...
            # ---  2. REPROJECT to local grid: we want to project
            # out_image with out_meta to local crs of glacier
                # Calculate Transform
            dst_transform, width, height = calculate_default_transform(
                    src1.crs, local_crs, out_image.shape[1],
                    out_image.shape[2], *src1.bounds)
    ...
    # Merge all bands to write into netcdf file!
    all_bands = xr.concat(b_sub, dim='band')
    all_bands['band'] = ['solar_azimuth','solar_zenith']
    all_bands.name = 'solar_angles'

    all_bands = all_bands.assign_coords(time=cfg.PARAMS['date'][0])
    all_bands = all_bands.expand_dims('time')

    # check if netcdf file for this glacier already exists, create if not, append if exists
    if not os.path.isfile(gdir.get_filepath('solar_angles')):
        print("netcdf does not exist yet, creating new")
        all_bands.to_netcdf(gdir.get_filepath('solar_angles'),
                            'w', format='NETCDF4',
                            unlimited_dims={'time': True})
    else:
        print('Open existing file')
        existing = xr.open_dataset(gdir.get_filepath('solar_angles'))
        # Convert all_bands from DataArray to Dataset
        all_bands= all_bands.to_dataset(name = 'solar_angles')
        if all_bands.time.values in existing.time.values:
            print("date already exists, not writing again")
        else:
            print("New date, appending to netcdf...")
            ...
            existing.close()
            #Write to file
            appended.to_netcdf(gdir.get_filepath('solar_angles'),
                               'w', format='NETCDF4',
                               unlimited_dims={'time': True})
